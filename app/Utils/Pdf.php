<?php

namespace App\Utils;

use App\Enums\EditorEnum;
use App\Models\Task;
use App\Models\TestRun;
use Gotenberg\Gotenberg;
use Gotenberg\Stream;
use Illuminate\Support\Facades\Storage;
use Psr\Http\Message\ResponseInterface;

class Pdf
{
    public static function generateResultPdfs(TestRun $testRun): array
    {
        // Create direcotry if missing
        $storage = Storage::disk('public');
        if (!$storage->exists("results/{$testRun->hash}")) {
            $storage->makeDirectory("results/{$testRun->hash}");
        }

        // Get the task with the highest step for each editor
        $finalTasks = [
            $testRun->tasks->where('editor', EditorEnum::CKEDITOR)->sortByDesc('step')->first(),
            $testRun->tasks->where('editor', EditorEnum::GRAPESJS)->sortByDesc('step')->first(),
        ];

        $files = [];
        foreach ($finalTasks as $task) {
            // Parse task content
            $html = static::renderDocument(static::replaceVariables($task?->html ?? '', $task?->variables ?? [], $task?->editor ?? null), $task?->css ?? '');

            // Convert and save
            $files[] = $storage->path("results/{$testRun->hash}") . '/' . Gotenberg::save(
                Gotenberg::chromium(config('gotenberg.gotenberg_api_url'))
                    ->pdf()
                    ->paperSize(8.27, 11.7) // A4 Format
                    ->margins(top: 0, right: 0, bottom: 0, left: 0) // Set via css
                    ->assets(Stream::string('style.css', $task?->css ?? ''))
                    ->outputFilename($task?->editor?->value ?? 'invalid')
                    ->html(Stream::string('index.html', $html)),
                $storage->path("results/{$testRun->hash}")
            );
        }

        return $files;

    }

    public static function getPdfAsResponse(string $html, string $css, array $data = []): ResponseInterface
    {
        $html = static::renderDocument(static::replaceVariables($html, $data), $css);

        return Gotenberg::send(
            Gotenberg::chromium(config('gotenberg.gotenberg_api_url'))
                ->pdf()
                ->paperSize(8.27, 11.7) // A4 Format
                ->margins(top: 0, right: 0, bottom: 0, left: 0) // Set via css
                ->assets(Stream::string('style.css', file_get_contents(resource_path('css/ckeditor-pdf.css'))))
                ->html(Stream::string('index.html', $html))
        );
    }

    public static function replaceVariables(string $html, array $data = [], ?EditorEnum $editor = null): string
    {
        if (!$editor || $editor === EditorEnum::CKEDITOR) {
            return preg_replace_callback('/{{\s*(.*?)\s*}}/', function ($matches) use ($data) {
                return $data[$matches[1]] ?? $matches[0]; // replace if exists, otherwise leave as-is
            }, $html);
        } else {
            return str_replace(
                search: 'src="http://localhost:8000/storage/assets/venuzle-logo.png"',
                replace: 'src="data:image/png;base64,"',
                subject: $html,
            );
        }
    }

    public static function renderDocument(string $html, string $css): string
    {
        return view(
            view: 'documents.html-skeleton',
            data: ['content' => $html, 'css' => $css],
        )->render();
    }
}
